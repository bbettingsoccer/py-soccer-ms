...
class ChampionshipService:
    ...
    def __init__(self):
        self.collection = OperationImplDAO("Championship_op")
    ...
    async def getChampionshipForCondition(self, search: str, values: [str]):
        ...
        match search:
            case MatchConstants.GET_SEARCH_CHAMPIONSHIP_NAME:
                filter = {ChampionshipModel.config.championship: {"$eq": values[0]}}
            case MatchConstants.GET_SEARCH_CHAMPIONSHIP_CODE:
                filter = {ChampionshipModel.config.code_name: {"$eq": values[0]}}
    ...
    async def deleteChampionshipForCondition(self, type: str, value: str):
        filter = []
        match type:
            case MatchConstants.DELETE_CHAMPIONSHIP_NAME:
                filter = {ChampionshipModel.config.championship: value}
        try:
            objFind = await self.getChampionshipForCondition(MatchConstants.GET_SEARCH_CHAMPIONSHIP_NAME,value)
            if objFind:
